...
import type { Tag, User } from "../models/index.ts";
import { authedRequester, buildAuthedRequester, generateFakeUser } from "../../test/index.ts";
...
describe("tag.controller", () => {
  ...
  beforeEach(async () => {
    author = await prisma.user.create({ data: {
      ...generateFakeUser(),
      role: "author"
    }});
  ...
  describe("[GET] /api/tags", () => {
    it("should return all tags from the database", async () => {
      const databaseTags = await prisma.tag.createManyAndReturn({ data: [
    ...
    describe("parameters: limit", () => {
      it("should return the limited number of tags", async () => {
        ...
        const { data: tags } = await authedRequester.get(`/tags?limit=${LIMIT}`);
    ...
    describe("parameters: page", () => {
      it("should return the tags from page the requested page", async () => {
        ...
        const { data: tags } = await authedRequester.get(`/tags?page=${PAGE}`);
    ...
    describe("parameters: author_id", () => {
      it("should only return the tags from the requested user", async () => {
        ...
        const { data: tags } = await authedRequester.get(`/tags?author_id=${AUTHOR_ID}`);
    ...
    describe("parameters: order", () => {
      it("should only tags in alphabetic order", async () => {
        ...
        const { data: tags } = await authedRequester.get(`/tags?order=name:asc`);
    ...
    async function generateMultipleTags(N: number, tag?: Partial<Tag>) {
      ...
      for (let i = 1; i <= N; i++) {
        const iTag = await prisma.tag.create({ data: {
          name: `Tag n°${i}`,
          author_id: author.id,
          ...tag
        } });
  ...
  describe("[GET] /api/tags/:id", () => {
    it("should return the requested tag with all basic properties", async () => {
      const databaseTag = await prisma.tag.create({ data: {
    ...
    it("should return the whole tag hierarchy", async () => {
      const devTag = await prisma.tag.create({ data: {
        name: "Web dev",
        author_id: author.id,
      }});
      const frontTag = await prisma.tag.create({ data: {
        name: "Frontend",
        author_id: author.id,
        parent_tag_id: devTag.id
      }});
      const reactTag = await prisma.tag.create({ data: {
        name: "React",
        author_id: author.id,
        parent_tag_id: frontTag.id
      }});
      const svelteTag = await prisma.tag.create({ data: {
        name: "Svelte",
        author_id: author.id,
        parent_tag_id: frontTag.id
      }});

      const { data: tag } = await authedRequester.get(`/tags/${frontTag.id}`);
    ...
    it("should return a 404 when the requested tag does not exist", async () => {
      ...
      const { status } = await authedRequester.get(`/tags/${UNEXISTING_ID}`);
  ...
  describe("[PATCH] /api/tags/:id", () => {
    ...
    beforeEach(async () => {
      tagToUpdate = await prisma.tag.create({ data: {
    ...
    it("should return the updated tag", async () => {
      ...
      const { status, data: tag } = await authedRequester.patch(`/tags/${tagToUpdate.id}`, { name: NEW_NAME });
    ...
    it("should effectively update the tag in database", async () => {
      ...
      await authedRequester.patch(`/tags/${tagToUpdate.id}`, { name: NEW_NAME });
  ...
  describe("[DELETE] /api/tags/:id", () => {
    ...
    it("should delete the tag if the requester is an author that created the tag", async () => {
      const tag = await prisma.tag.create({ data: { name: "Niveau à supprimer", author_id: author.id } });
      const httpRequester = buildAuthedRequester(author);
